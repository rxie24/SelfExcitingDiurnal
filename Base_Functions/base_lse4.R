#Base Functions for Linear Self-Exciting Log-Likelihoods and Full LSE Model Optimization w/ Constraint on Waiting Times

#Modified function to calculate lambda at multiple time points (inverse of waiting time)
cppFunction("
std::vector<double> compute_lambda_lse(const std::vector<double>& t, const std::vector<double>& times, double mu, const std::vector<double>& beta, const std::vector<double>& eta, const std::vector<double>& gammas) {
    ...
    double diff0 = std::max(0.01,(t - times[nt2 - 1]));
    se_sum += beta[0] / diff0;
    
    for (int i = 1; i < p; ++i) {
        double diff = times[nt2-i] - times[nt2-i-1];
        diff = std::max(0.01,diff);
        se_sum += beta[i] / diff;
    } 
