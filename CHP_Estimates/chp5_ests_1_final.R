...
ids <- c(1, 6, 7, 8, 10, 24, 28, 30, 40)
ncores <- length(ids)

#Fit CHP Models for individuals
all_lls <- mclapply(ids,chp_llest,wd=wd,wddata=wddata,nfc=nfc,ip=0.9,mc.cores=ncores) #have no roughness penalty
print(all_lls)
...
saveRDS(object = id_lls,file=paste0(wd,"CHP_ID_LLs1.rds")) 
