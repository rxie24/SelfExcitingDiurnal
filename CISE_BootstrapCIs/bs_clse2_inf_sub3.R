...
wdest <- "/home/xieryan/Dissertation1/Data/CLSE2/"
...
for(j in 21:36){
  ...
  mles_lse <- readRDS(paste0(wdest,"ests",j,".rds"))
  print(mles_lse)
  
  #Now perform bootstrapping to try and get confidence intervals
  tryCatch({
    mle_bs_ci <- lse_par_bootstrap(mles_lse,n,b=2,ncores=ncores,niter=niter)
  },
  error=function(e){
    mle_bs_ci <<- rep(NaN,5)
  })
